...
<div style={{margin: "0 0 32px", padding: "48px 32px 44px", borderRadius: "16px", background: "#0b1a22", backgroundImage: "radial-gradient(ellipse 900px 400px at 50% 0%, rgba(16,163,174,0.28) 0%, transparent 70%)", textAlign: "center", border: "1px solid rgba(16,163,174,0.18)", overflow: "hidden"}}>
  ...
  <div style={{margin: "0 0 16px", fontSize: "clamp(26px, 5vw, 52px)", fontWeight: 700, letterSpacing: "-0.03em", lineHeight: 1.05, color: "#f0f6f8"}}>The vector database that keeps<br />your data encrypted at rest, in transit, and in use.</div>
  <div style={{margin: "0 auto", maxWidth: "580px", fontSize: "16px", lineHeight: 1.6, color: "#8ba8b5"}}>Similarity search directly on encrypted embeddings — self-hosted in your VPC or on-prem. AES-256-GCM with your KMS key, per-operation key derivation, keys zeroized after use.</div>
</div>

**CyborgDB** is an encrypted vector database — similarity search directly on encrypted embeddings, with per-operation key derivation via your KMS and keys zeroized after use. Only the final candidates are decrypted. It's a self-hosted, standalone vector database — persist to embedded RocksDB (default) or S3 — exposes a familiar API that slots into existing AI pipelines, and can be [GPU-accelerated with NVIDIA cuVS](https://developer.nvidia.com/blog/bringing-confidentiality-to-vector-search-with-cyborg-and-nvidia-cuvs/).
...
<CardGroup cols={3}>
    <Card title="Learn the Fundamentals" href="versions/v0.17.x/intro/about" icon="book-open-cover">
        Understand the architecture and principles behind encrypted vector search
    </Card>
    <Card title="Start Building" href="versions/v0.17.x/intro/quickstart" icon="rocket-launch">
        Get hands-on with CyborgDB in minutes using our quickstart guide
    </Card>
    <Card title="Explore Deployment Options" href="versions/v0.17.x/intro/deployment-models" icon="server">
...
<CardGroup cols={3}>
    <Card title="Introduction" href="versions/v0.17.x/intro/about" icon="graduation-cap">
        Core concepts, deployment models, and getting started guides
    </Card>
    <Card title="CyborgDB Service" href="versions/v0.17.x/service/guides/intro/about" icon="cloud">
        CyborgDB self-hosted with REST API and client SDKs
    </Card>
    <Card title="CyborgDB Embedded" href="versions/v0.17.x/embedded/guides/intro/about" icon="code">
        Self-hosted, embedded deployments with Python and C++ bindings
    </Card>
</CardGroup>

<CardGroup cols={2}>
    <Card title="Integrations" href="versions/v0.17.x/integrations/about" icon="plug">
        LangChain and other framework integrations for seamless adoption
    </Card>
    <Card title="How to Use These Docs" href="versions/v0.17.x/intro/using-docs" icon="map">
