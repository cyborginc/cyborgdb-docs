...
<Note>
**Storage surface.** The service exposes a single, flat storage
surface: `memory`, `disk`, or `s3` (AWS S3 or any S3-compatible
store). Configure it with `CYBORGDB_DB_TYPE` plus the matching
`CYBORGDB_DISK_PATH` / `CYBORGDB_S3_*` variables.
</Note>
...
|----------|-------------|---------|
| `CYBORGDB_API_KEY` | _(Optional)_ Your CyborgDB license key, forwarded to the core engine. If unset, the service runs in **free tier** mode (per-index cap of 1,000,000 items). Set a paid key for unlimited usage. This is **not** a service-authentication credential — it does not gate access to the REST API. | `cyborg_abc123...` |
...
This key controls licensing only. Request authentication is a separate concern, gated by `CYBORGDB_SERVICE_ROOT_KEY` (see [RBAC / Multi-Tenancy](#rbac-multi-tenancy)).
</Note>
...
|----------|-------------|---------|
| `CYBORGDB_SERVICE_ROOT_KEY` | When set, enables authentication and RBAC. The holder of this key has full admin access and can mint per-user API keys via `POST /v1/indexes/{index_name}/users`. Per-user keys are scoped to one index with `read` / `write` permissions enforced cryptographically. When unset, authentication is disabled. | unset (auth off) |
...
Persistent local key-value storage with no external dependency.

| Variable | Description | Default |
|----------|-------------|---------|
| `CYBORGDB_DISK_PATH` | Data directory for the disk store | `~/.cyborgdb/data` (local) / `/app/cyborgdb_data` (Docker) |
...
<AccordionGroup>
    ...
    <Accordion title="`Invalid CYBORGDB_DB_TYPE`">
        CyborgDB accepts `memory`, `disk`, or `s3`. Set
        `CYBORGDB_DISK_PATH` if you use a custom directory for the
        disk store.
    </Accordion>
