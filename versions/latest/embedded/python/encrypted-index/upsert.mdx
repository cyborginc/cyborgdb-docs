...
| `contents` | `bytes` or `str` | Item contents (_optional_). Accepts strings or bytes; all are encoded to bytes and encrypted before storage. Must be a `str` if index is configured to compute embeddings. |
