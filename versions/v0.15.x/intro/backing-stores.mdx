...
RocksDB provides persistent key-value storage directly on local disk, with no network dependency. This makes it the recommended backing store for embedded/local CyborgDB deployments where simplicity and reliability are priorities.
...
**Thread-safe in-memory storage for concurrent workloads**

ThreadSafeMemory extends the in-memory backing store with thread-safe access, allowing concurrent reads and writes from multiple threads without external synchronization.

### Key Benefits

**Thread-Safe Access**
Built-in locking ensures safe concurrent access from multiple threads.

**Zero Network Latency**
Same performance characteristics as Memory storage with added thread safety.

**Multi-threaded Benchmarking**
Ideal for evaluating CyborgDB performance under concurrent workloads.

**Best For:** Multi-threaded benchmarking, concurrent testing environments, and applications that need ephemeral thread-safe storage.
...
**Zero Configuration**
Starts automatically when CYBORGDB_DB_TYPE is not set - no external database setup required.
