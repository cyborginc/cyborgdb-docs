...
<Tip>You can adjust the number of vectors that will trigger automatic training by setting the `RETRAIN_THRESHOLD` environment variable. See more in the [Environment Variables](/env-vars.mdx) guide.</Tip>
...
| `max_memory` | `int` | `None` (0) | _(Optional)_ Maximum memory usage in MB. When `None` or 0, no memory limit is applied. |
