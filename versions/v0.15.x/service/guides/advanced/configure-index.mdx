...
| [`IVFSQ`](#ivfsq-index-type) | Fast | High |  Small  |
...
<Note>The `IVFSQ` index type is only supported in paid plans.</Note>
...
// Create the IVFSQ index config
const indexConfig = new IndexIVFSQ({ sqBits: 16 });
...
`sq_bits` is the number of bits per dimension used for scalar quantization. The default value is `16`. Lower values yield smaller index sizes but may reduce recall.
