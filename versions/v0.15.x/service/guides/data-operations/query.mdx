...
- `include`: item fields to return (e.g., `["distance", "metadata"]`). Default: `["distance", "metadata"]`. Note: `id` is always included.
- `greedy`: whether to use faster approximate search. Default: `None` (server determines behavior).
