...
<Tip>In Python, the `contents` field is returned in its original format (string or bytes) as it was stored. String contents remain strings, and bytes contents remain bytes after decryption. In JavaScript/TypeScript, contents are returned as decoded UTF-8 strings. In Go, contents are returned as []byte.</Tip>
