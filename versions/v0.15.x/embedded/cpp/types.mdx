...
| `table_name` | `std::string` | _(Optional)_ Name of the table in the database, if applicable. Defaults to `std::nullopt`. |
| `db_connection_string` | `std::string` | _(Optional)_ Connection string for database access, if applicable. Defaults to `std::nullopt`. |
...
| `embedding_model` | `std::string` | `""` | _(Optional)_ Embedding model name for auto-generation. |
