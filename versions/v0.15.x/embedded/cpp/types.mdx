...
| `embedding_model` | `std::optional<std::string>` | `std::nullopt` | _(Optional)_ Embedding model name for auto-generation. |
...
|--------------|-------------------------------|---------|-------------------------------------------------------------------------------|
| `dimension` | `size_t` | `0` | _(Optional)_ Dimensionality of vector embeddings. Auto-detected if 0. |
| `pq_dim` | `size_t` | `0` | _(Optional)_ Dimensionality of embeddings after quantization. Auto-detected if 0. |
| `pq_bits` | `size_t` | `8` | _(Optional)_ Number of bits per dimension for PQ embeddings (between 1 and 16). |
| `embedding_model` | `std::optional<std::string>` | `std::nullopt` | _(Optional)_ Embedding model name for auto-generation. |
...
```cpp
IndexIVFSQ(size_t dimension = 0, size_t sq_bits = 8,
           std::optional<std::string> embedding_model = std::nullopt);
```
...
|--------------|-------------------------------|---------|-------------------------------------------------------------------------------|
| `dimension` | `size_t` | `0` | _(Optional)_ Dimensionality of vector embeddings. Auto-detected if 0. |
| `sq_bits` | `size_t` | `8` | _(Optional)_ Number of bits for scalar quantization (typically 4, 8, or 16). |
| `embedding_model` | `std::optional<std::string>` | `std::nullopt` | _(Optional)_ Embedding model name for auto-generation. |
...
```cpp
// Default configuration (8-bit scalar quantization)
cyborg::IndexIVFSQ config1;

// 16-bit scalar quantization (higher precision)
cyborg::IndexIVFSQ config2(0, 16);

// Explicit dimension and SQ bits
cyborg::IndexIVFSQ config3(128, 8);
```
...
| `batch_size` | `std::optional<size_t>` | _(Optional)_ Size of each batch for training. Defaults to `std::nullopt` (auto-determines, typically `2048`). |
...
|--------------------------|------------------------------|------------------------------------------------------|
| `ids` | `std::vector<std::vector<std::string>>` | IDs of nearest neighbors for each query. |
| `distances` | `std::vector<std::vector<float>>` | Distances of nearest neighbors for each query. |
| `metadata` | `std::vector<std::vector<std::string>>` | Metadata for nearest neighbors for each query (JSON strings). |
