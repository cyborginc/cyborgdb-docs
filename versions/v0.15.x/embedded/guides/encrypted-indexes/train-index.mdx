...
|---------------|--------|---------|-------------------------------------------------------|
| `n_lists` | `int` | `auto` | _(Optional)_ Number of inverted index lists to create in the index. Auto-determines based on the number of vectors in the index. |
| `batch_size` | `int` | `auto` | _(Optional)_ Size of each batch for training. Auto-determined based on dataset size. |
| `max_iters` | `int` | `auto (100)` | _(Optional)_ Maximum number of iterations for training. Defaults to `100`. |
