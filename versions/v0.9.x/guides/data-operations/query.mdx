...
- `include`: a list of item fields to return (e.g., `["distances", "metadatas"]`).
...
std::vector<std::string> include = {"distances", "metadatas"};

// Perform query
cyborg::QueryParams query_params(top_k, n_probes, greedy, filters, include);
cyborg::QueryResults results = index.Query(query_vector, query_params);

// Print the results
for (const auto& result : results) {
    std::cout << "ID: " << result.id << ", Distance: " << result.distance << ", Metadata: " << result.metadata << std::endl;
}
```
</CodeGroup>
...
<Note>This feature is only available in Python.</Note>
