...
| `include` | `std::vector<std::string>` | _(Optional)_ List of item fields to return. Can include `distance`, `metadata`, and `vector` (`vector` only for `IVFFlat` indexes). Defaults to `"distance"` |
