...
enum class Location {
    ...
    kS3,                // AWS S3 or any S3-compatible object store
...
| `table_name` | `std::string` | _(Optional)_ Reserved; no current `Location` uses it in v0.17. Defaults to `std::nullopt`. |
| `db_connection_string` | `std::string` | _(Optional)_ Connection string or path for the chosen location (e.g. RocksDB data directory, S3 URI). Defaults to `std::nullopt`. |
...
// S3 (cloud-native)
cyborg::DBConfig s3_loc(Location::kS3, std::nullopt, "s3://my-bucket?region=us-east-1");

// In-memory (testing only)
cyborg::DBConfig mem_loc(Location::kThreadSafeMemory);
```
...
|--------------|-------------------------------|---------|-------------------------------------------------------------------------------|
| `dimension` | `size_t` | `0` | _(Optional)_ Dimensionality of vector embeddings. Auto-detected if 0. |
| `sq_bits` | `size_t` | `16` | _(Optional)_ Number of bits for scalar quantization (8 or 16). |
| `embedding_model` | `std::string` | `""` | _(Optional)_ Embedding model name for auto-generation. |
