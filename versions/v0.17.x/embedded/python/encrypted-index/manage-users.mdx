...
results = index.query(query_vectors=[0.1, 0.2, 0.3], top_k=5, index_key=user_kek, user_id=user_id)

# Denied: a read-only user cannot write — this raises
index.upsert([{"id": "item_1", "vector": [0.1, 0.2, 0.3]}], index_key=user_kek, user_id=user_id)
```
