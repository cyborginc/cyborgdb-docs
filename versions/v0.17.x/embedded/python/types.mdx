...
|-------------------|----------------------------------|--------------|--------------------------------------------------------------------|
| `location` | `string` | - | DB location (`rocksdb`, `s3`, `memory`, `threadsafememory`) |
| `connection_string` | `string` | None | _(Optional)_ Connection string or path for the chosen location. |
...
- `"s3"`: Use for AWS S3 or any S3-compatible object store (MinIO, Cloudflare R2). Carry the URI and credentials in `connection_string`.
...
<Warning>`memory` is deprecated and will be removed in a future release. Please use `threadsafememory` instead.</Warning>
...
index_location = cyborgdb.DBConfig(location="rocksdb")
config_location = cyborgdb.DBConfig(location="rocksdb")

# S3-backed storage (cloud-native)
s3_location = cyborgdb.DBConfig(
    location="s3",
    connection_string="s3://my-bucket?region=us-east-1"
)
```
...
# Basic configuration with auto-detection (default 16-bit SQ)
