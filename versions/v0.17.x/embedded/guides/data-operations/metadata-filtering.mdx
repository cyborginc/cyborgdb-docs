...
# Upsert data with metadata (index_key is required on every data operation)
index.upsert(data, index_key=index_key)
```
...
# Perform query
results = index.query(query_vectors=query_vectors, top_k=top_k, filters=filters, index_key=index_key)
```
