...
In Python, results are returned as a `list[dict]` with string keys — access fields with `r["id"]`, `r["distance"]`, and `r["metadata"]`. A batch query returns a `list[list[dict]]`.
...
# Perform query (index_key is required on every data operation)
results = index.query(query_vectors=query_vectors, top_k=top_k, index_key=index_key)

print(results)
# Example results (list[dict] with string keys)
# [{"id": "item_12"}, {"id": "item_7"}, ...]
...
// Perform query (index_key implicitly builds the KeyContext)
...
- `rerank_mult`: stage-1 retrieval multiplier for the reranking pass on `DiskIVF` indexes (default `10`). Higher values retrieve more candidates before the float32/float16 rerank, increasing recall at some latency cost.
...
for r in results:
    print(f"ID: {r['id']}, Distance: {r['distance']}, Metadata: {r['metadata']}")
```
...
std::vector<cyborg::ResultFields> include = {cyborg::ResultFields::kDistance, cyborg::ResultFields::kMetadata};
size_t rerank_mult = 10;

// Perform query (params + key are required)
cyborg::QueryParams query_params(top_k, n_probes, filters, include, greedy, rerank_mult);
cyborg::QueryResults results = index->Query(query_vectors, query_params, index_key);

// Print the results
auto view = results[0];
for (uint32_t i = 0; i < view.num_results; ++i) {
    std::cout << "ID: " << view.ids[i] << ", Distance: " << view.distances[i] << ", Metadata: " << view.metadata[i] << std::endl;
}
```
</CodeGroup>
