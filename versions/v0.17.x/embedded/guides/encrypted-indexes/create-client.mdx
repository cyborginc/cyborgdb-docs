...
# `s3`, `memory`, and `threadsafememory` are also supported.
...
// Using `rocksdb` for persistent local storage.
// `kS3`, `kMemory`, and `kThreadSafeMemory` are also supported.
...
Bear in mind that all contents stored in the backing stores is end-to-end encrypted, meaning that **no index contents will be stored in plaintext**.
...
# Using `rocksdb` for the index/config and `s3` for the items
index_location = cyborgdb.DBConfig(location="rocksdb")
config_location = cyborgdb.DBConfig(location="rocksdb")
item_location = cyborgdb.DBConfig(location="s3", connection_string="s3://my-bucket?region=us-east-1")
...
// Using `kRocksDB` for the index/config and `kS3` for the items
cyborg::DBConfig index_location(cyborg::Location::kRocksDB);
cyborg::DBConfig config_location(cyborg::Location::kRocksDB);
cyborg::DBConfig contents_location(cyborg::Location::kS3, std::nullopt, "s3://my-bucket?region=us-east-1");
...
Item contents will be encrypted prior to being uploaded to the backing store. In most applications, these are accessed less frequently than the index, so a higher-latency / lower-cost option such as S3 is often a good fit.
