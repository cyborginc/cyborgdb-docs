...
A user loads the index with their own KEK as the `index_key` and passes their `user_id`. From then on, every operation is gated to the permissions they hold — a read-only user's `upsert`/`delete` will be rejected.
...
# user_index.upsert([...])  # raises — no write wrap
```
...
// Writes are rejected for a read-only user (no write wrap)
```
</CodeGroup>
