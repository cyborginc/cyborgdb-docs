...
<Steps>
    ...
    <Step title="Create a Client">
        ...
        # Persistent local storage (RocksDB on disk). For cloud / multi-replica
        # deployments, swap `disk` for an `s3`-compatible object store (AWS S3,
        # MinIO, Cloudflare R2, …).

        index_location = cyborgdb.StorageConfig("disk")   # Where encrypted index is stored (for queries)
        config_location = cyborgdb.StorageConfig("disk")  # Where encrypted index config is stored (for config/loading)
        items_location = cyborgdb.StorageConfig("disk")   # Where item contents are stored (for upsert/get)
        ...
        // Persistent local storage (RocksDB on disk). For cloud / multi-replica
        // deployments, swap `kDisk` for `kS3` to target AWS S3 or any
        // S3-compatible object store (MinIO, Cloudflare R2, …).

        cyborg::StorageConfig index_location(cyborg::Location::kDisk);    // Where encrypted index is stored (for queries)
        cyborg::StorageConfig config_location(cyborg::Location::kDisk);   // Where encrypted index config is stored (for config/loading)
        cyborg::StorageConfig contents_location(cyborg::Location::kDisk); // Where item contents are stored (for upsert/get)
