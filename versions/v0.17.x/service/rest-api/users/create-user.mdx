...
Mint a per-user API key scoped to a single index. Requires RBAC enabled on the service (`CYBORGDB_ROOT_API_KEY` set) and the request must be authenticated with the root key.
...
- `403`: RBAC is not enabled (`CYBORGDB_ROOT_API_KEY` unset), or the caller is not using the root key.
- `404`: Index not found.
