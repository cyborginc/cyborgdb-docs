...
| `top_k` | `int` | `None` | _(Optional)_ Number of results to return per query. When `None`, the server determines the default (typically 100) |
...
| `greedy` | `bool` | `None` | _(Optional)_ Use greedy search algorithm. When `None`, the server determines the default behavior |
