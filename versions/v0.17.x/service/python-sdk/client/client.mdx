...
<Note>`api_key` is **only required when the service has authentication enabled** — i.e. `CYBORGDB_SERVICE_ROOT_KEY` is set on the service. When the service runs unauthenticated (the default), you can omit `api_key`.</Note>
