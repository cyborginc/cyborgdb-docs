...
When the service runs with `CYBORGDB_ROOT_API_KEY` set, the Go SDK exposes per-index user provisioning on `*EncryptedIndex`. These calls require the client to be using the root API key.
