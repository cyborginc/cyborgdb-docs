...
<Steps>
    ...
    <Step title="Run with Docker (Quick Start)">
        <Tabs>
            <Tab title="Disk (default)">
                <CodeGroup>
                ```bash Linux
                # Minimal — no API key needed; disk is the default; bind the data dir to a host volume
                ...
                <Note>Inside the container the disk path defaults to `/app/cyborgdb_data` — the `-v` mount above is what makes the data survive container recreation. Add `-e CYBORGDB_API_KEY=…` to lift the free-tier 1M-items-per-index cap.</Note>
            </Tab>
            ...
            <Tab title="Disk (default)">
                ...
                services:
                  cyborgdb:
                    ...
                    ports:
                      - "8000:8000"
                    # No API key needed; add CYBORGDB_API_KEY to lift the free-tier cap:
                    # environment:
    ...
    <Step title="Run Your First Query">
        ...
        # Query — include distance + metadata, then chain get() for contents.
        results = index.query(
        ...
        **Store `index_key` before you upsert any data.** CyborgDB has no key-recovery path — data encrypted with a lost key is unrecoverable. For anything past evaluation, keep the key in AWS Secrets Manager, Vault, or your KMS. See [Managing Keys](../advanced/managing-keys).
        ...
        <Tabs>
            <Tab title="Environment Variables">
                ...
                <Note>None of these are required — the container starts with no configuration, using `disk` storage rooted at `/app/cyborgdb_data`, free-tier licensing, and authentication disabled. For the full list (including S3 credentials and KMS), see the [Environment Variables](../advanced/env-vars) guide.</Note>
