...
Native SDKs for Python, JavaScript/TypeScript, and Go - use your preferred language stack.
...
- **Horizontal Scaling**: Deploy multiple service instances behind a load balancer (use the `s3` backend so they share state)
- **Caching**: Opt in to per-keystore RAM caches via `CACHE_POLICY_VECTORS`, `CACHE_POLICY_METADATA`, and `CACHE_POLICY_IDS` (see [Environment Variables](../advanced/env-vars))
- **GPU Acceleration**: Set `GPU_OPERATIONS=upsert,train` (or `all`) on a CUDA host to offload index ops
...
- **Storage & KMS Credentials**: Restrict S3 access via IAM and protect KMS provider credentials — see [Managing Encryption Keys](../advanced/managing-keys)
