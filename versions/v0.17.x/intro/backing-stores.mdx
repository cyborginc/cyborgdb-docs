...
<Note>
**Service vs. Embedded backends differ in v0.17.** CyborgDB Service
(REST API) exposes a flat surface: `memory`, `disk` (RocksDB), or
`s3`. The embedded libraries (Python / C++) additionally support
PostgreSQL and Redis via `DBConfig(...)`. Each backend below notes
which deployment model it applies to.
</Note>
...
*Available in: Embedded only* — not supported by CyborgDB Service v0.17.
...
<Tabs>
  ...
  <Tab title="Use PostgreSQL (embedded)">
    ...
    *Note: Embedded-only in v0.17. For Service deployments, use `s3` instead.*
  </Tab>

  <Tab title="Use Redis (embedded)">
