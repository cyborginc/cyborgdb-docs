...
<Note>
**Storage surface.** CyborgDB supports `memory`, `disk`, and `s3` (AWS S3 or any S3-compatible store) across both deployment models.
</Note>
...
<Note>The embedded libraries and LangChain integration select this backend with the literal value `disk`. CyborgDB Service selects it via `CYBORGDB_DB_TYPE=disk` (and it is the default).</Note>

**Persistent local storage for single-node deployments**

The disk backend provides persistent key-value storage directly on local disk, with no network dependency. This makes it the recommended backing store for single-node CyborgDB deployments where simplicity and reliability are priorities.
...
# Local disk (default path: ~/.cyborgdb/data)
db_config = DBConfig("disk")
```
...
| **Disk** | ~10µs | High | Local Disk | Single Process |
