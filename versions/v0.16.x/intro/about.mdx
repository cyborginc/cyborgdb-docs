...
**CyborgDB solves this** by enabling Approximate Nearest-Neighbor (ANN) search directly over **encrypted vectors**. Your embeddings are stored and indexed as ciphertext — only the final candidates are decrypted.
...
**The Process:**
1. **Your app sends vectors** → CyborgDB encrypts them with your key, inside your VPC or on-prem (AES-256-GCM)
...
5. **Results returned** → Decrypted matching vectors and metadata, returned over TLS

**Key Innovation:** Traditional vector databases decrypt data during search. CyborgDB performs similarity search directly on encrypted vectors — only the final candidates are decrypted.
...
**🔒 Encrypted Storage, Search & Transit**  
Vector embeddings stay encrypted at rest, in transit (TLS), and during index traversal. Only the final candidates are decrypted.
