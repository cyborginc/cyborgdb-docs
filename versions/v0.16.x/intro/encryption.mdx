---
title: "Encryption"
canonical: https://docs.cyborg.co/versions/latest/intro/encryption
---

CyborgDB encrypts **all sensitive data** — including embeddings, document IDs, contents, and metadata — ensuring that nothing is stored in plaintext. Only the final candidates are decrypted. Keys are derived per-operation from your KMS and zeroized after use. This section details the cryptographic design, the indexing scheme, and how forward privacy is achieved.
...
4. **Encrypted index traversal**  
   Queries traverse the index over ciphertext nodes. Only the final candidates are decrypted for reranking.
...
This ensures that the server can execute efficient ANN search over ciphertext. Only the final candidates are decrypted for reranking; keys are zeroized after each operation.

In embedded deployments, decryption happens inside the application process. In service deployments, the service decrypts and returns results over TLS. 
...
| **Network interception** | TLS encryption in transit                                                         |
| **Server compromise**    | Encrypted index and storage; only final candidates are decrypted during active queries |
...
|----------|----------------|---------|
| **Query Privacy** | Token-scoped key derivation | Keys are derived per-operation and zeroized after use |
| **Result Confidentiality** | TLS transport + encrypted storage | Backing store never holds plaintext results; results returned over TLS |
