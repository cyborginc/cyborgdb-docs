...
# mode: "wide"
canonical: https://docs.cyborg.co/versions/latest/intro/security-overview
---
...
<Tip>
A well-scoped threat model is essential to evaluate whether CyborgDB's security properties align with your risk profile. This page should be read alongside the detailed [Encryption](./encryption) and [Threat Model](./threat-model) sections.
</Tip>
...
```mermaid
graph TD
    A[CyborgDB Core Principles] --> B[Ciphertext-Only Storage]
    A --> C[Forward Privacy]
    A --> D[Key Isolation]
    
    B --> F[Encrypted Index Traversal]
    B --> G[Per-Operation Key Derivation]
...
- **Encrypted index search**: Index traversal operates over ciphertext; only the final candidates are decrypted
...
<Tip>To learn more about how CyborgDB implements these protections, read the [Encryption guide](./encryption).</Tip>

## 4. Security Guarantees

CyborgDB provides mathematically provable protections against the threat scenarios in the [threat model](./threat-model).
...
| **Memory scraping** | ❌ Plaintext in RAM | ✅ Encrypted storage & index; only final candidates decrypted during active queries | Reduced exposure |
...
By protecting data across its full lifecycle — ciphertext-only storage, encrypted index traversal, and per-operation key derivation from your KMS — CyborgDB ensures that you can innovate with AI without creating a breach magnet for your organization's most valuable intelligence.
...
<CardGroup cols={3}>
    ...
    <Card title="Embedding Inversion" href="./embedding-inversion" icon="rotate-left">
