---
title: "Threat Model"
canonical: https://docs.cyborg.co/versions/latest/intro/threat-model
---
...
CyborgDB do **not** assume the client endpoint is compromised (see [Out of Scope](#6-out-of-scope)), nor does it defend against malicious users with valid credentials performing authorized actions.
...
The **Server-Side Adversary** case — full memory, disk, and runtime access — is the primary driver for CyborgDB's *encrypted index traversal* and *forward-secure index* design. Storage, backups, and network hold only ciphertext. Residual exposure: an adversary with live memory access during an active query could observe the decrypted final candidates and ephemeral keys.
...
For a deep dive on how embedding inversion works, the specific attack vectors, and comparative results across vector databases, see [Embedding Inversion](./embedding-inversion).
...
| **Memory scraping** | Encrypted index traversal with ephemeral node keys | Only final candidates and ephemeral keys are exposed during active queries |
...
| **Embedding inversion** | Encrypted embeddings at rest; only final candidates decrypted during queries | Compromised client could still expose |
