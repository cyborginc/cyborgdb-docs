...
canonical: https://docs.cyborg.co/versions/latest/intro/embedding-inversion
---
...
CyborgDB eliminates the inversion attack surface by ensuring embeddings are **never stored in plaintext**. Only the final candidates are decrypted during queries:

- **Encryption at rest**: All embeddings are encrypted with AES-256-GCM before being written to the backing store
- **Encrypted index search**: Index traversal operates on ciphertext; only the final candidates are decrypted for reranking
...
- **KMS integration**: Keys are protected via integration with AWS KMS, Azure Key Vault, and GCP KMS for hardware-backed security and audit logging
