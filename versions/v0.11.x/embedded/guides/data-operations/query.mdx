...
```cpp C++
// Example query
std::vector<float> query_vectors = {0.5, 0.9, 0.2, 0.7};
size_t top_k = 10;

// Perform query
cyborg::QueryParams query_params(top_k);
std::vector<cyborg::QueryResult> results = index->Query(query_vectors, query_params);
...
# Perform query
results = index.query(query_vectors=query_vectors, top_k=top_k, n_probes=n_probes, filters=filters, include=include)
...
// Perform batch query
cyborg::QueryParams query_params(top_k);
std::vector<std::vector<cyborg::QueryResult>> results = index->Query(query_vectors, query_params);
```
</CodeGroup>
