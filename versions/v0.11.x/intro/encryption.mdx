...
CyborgDB also encrypts the **search index** used for approximate nearest neighbor (ANN) retrieval. The index is constructed using a **forward-secure** cryptographic scheme inspired by research in forward-private searchable encryption, combined with our patented enhancements.
...
CyborgDB's search execution maintains cryptographic protection throughout the entire query process, leveraging our cryptographic index discussed [above](#2-forward-secure-cryptographic-indexing).
...
CyborgDB's multi-layered cryptographic design provides comprehensive protection against various threat scenarios, leveraging our patented technology to achieve security properties impossible with traditional approaches.
...
Our optimizations deliver industry-leading performance:
