...
| `include` | [`std::vector<cyborg::ItemFields>`](../types#itemfields) | _(Optional)_ List of item fields to return. Can include `kVector`, `kContents`, and `kMetadata` (`vector` only for [`IVFFlat`](../types#IndexIVFFlat) indexes). |
