...
```typescript
function validateVectorItem(item: VectorItem, expectedDimension: number): string[] {
    const errors: string[] = [];
    
    // Validate ID
    if (!item.id || typeof item.id !== 'string') {
        errors.push('ID is required and must be a string');
    } else if (item.id.length > 255) {
        errors.push('ID must be 255 characters or less');
    }
    ...
    if (item.vector) {
        ...
        } else if (item.vector.length !== expectedDimension) {
            errors.push(`Vector must have exactly ${expectedDimension} dimensions, got ${item.vector.length}`);
        } else if (item.vector.some(v => typeof v !== 'number' || isNaN(v))) {
            errors.push('All vector components must be valid numbers');
        }
    }
    ...
    // Validate contents
    if (item.contents && typeof item.contents !== 'string' && !Buffer.isBuffer(item.contents)) {
        errors.push('Contents must be a string or Buffer');
    }
...
async function safeUpsert(
    ...
    const validationErrors: { id: string, errors: string[] }[] = [];
    
    items.forEach(item => {
        const errors = validateVectorItem(item, expectedDimension);
        if (errors.length > 0) {
            validationErrors.push({ id: item.id, errors });
        }
    });
    ...
    // All items valid, proceed with upsert
    try {
        const result = await index.upsert(items);
        console.log(`Safe upsert completed: ${result.message}`);
        return result;
    } catch (error) {
        console.error('Safe upsert failed:', error.message);
        throw error;
    }
}

// Usage
const itemsToValidate: VectorItem[] = [
    { id: 'valid_item', vector: new Array(768).fill(0.1), contents: 'Valid content' },
    { id: 'invalid_item', vector: new Array(100).fill(0.1), contents: 'Wrong dimensions' }
];

try {
    await safeUpsert(index, itemsToValidate, 768);
} catch (error) {
    console.error('Validation or upsert failed:', error.message);
}
```
...
class IncrementalLoader {
    ...
    constructor(index: EncryptedIndex, batchSize: number = 100) {
        this.index = index;
        this.batchSize = batchSize;
    }
    
    async loadData(items: VectorItem[]) {
        console.log(`Starting incremental load of ${items.length} items`);
    ...
    private async processBatch(batch: VectorItem[], batchNumber: number) {
        const startTime = Date.now();
        ...
        console.log(`Batch ${batchNumber}: ${batch.length} items in ${duration}ms`);
        console.log(`Progress: ${this.processed} items processed`);
        
        return result;
    }
    
    private getLoadingSummary() {
        return {
    ...
    async trainAfterLoading() {
        if (this.processed > 0 && this.errors.length === 0) {
            console.log('All data loaded successfully, starting training...');
            try {
                const trainResult = await this.index.train();
                console.log('Training completed:', trainResult.message);
                return trainResult;
            } catch (error) {
            ...
            console.log('Skipping training due to loading errors or no data processed');
        }
    }
}

// Usage
const loader = new IncrementalLoader(index, 75);
const loadingResult = await loader.loadData(largeDataset);

if (loadingResult.success) {
    await loader.trainAfterLoading();
}

console.log('Loading summary:', loadingResult);
```
