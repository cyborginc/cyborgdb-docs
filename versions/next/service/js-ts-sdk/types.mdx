...
| `storagePrecision` | `'float32' \| 'float16' \| 'tq12' \| 'tq8' \| 'tq6' \| 'tq4'` | `'float32'` | On-disk rerank-vector format, chosen at create time and immutable. The TurboQuant tiers (12/8/6/4 bits per dimension) trade storage for a small recall/latency cost; `'tq4'` is the most aggressive (~8× smaller than `float32`, ~94% recall@100). All tiers work with every metric. |
...
There is no public `TrainingStatus` type in v0.17. Use `index.isTraining()` on an `EncryptedIndex` instance — it returns a `Promise<boolean>` indicating whether _this_ index is currently being trained (or queued for training) on the server.
