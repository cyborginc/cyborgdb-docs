...
<Tip>The `contents` field is returned verbatim as a string, exactly as stored — matching the Python SDK. Text contents round-trip as the original UTF-8 string. Contents that were upserted as a `Buffer` are base64-encoded by `upsert()` and round-trip as that base64 string; decode them with `Buffer.from(contents, "base64")` to recover the original bytes.</Tip>
...
| `contents` | `string` | The stored content, returned verbatim as a string (included if `"contents"` in include array). Base64-encoded when the original upsert passed a `Buffer` |
