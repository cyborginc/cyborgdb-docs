...
| `storage_precision` | `str` | `"float32"` | On-disk rerank-vector format, chosen at create time and immutable. One of `"float32"`, `"float16"`, or the TurboQuant tiers `"tq12"` / `"tq8"` / `"tq6"` / `"tq4"` (12/8/6/4 bits per dimension). TurboQuant trades storage for a small recall/latency cost; `"tq4"` is the most aggressive (~8× smaller than `float32`, ~94% recall@100). All tiers work with every metric. |
