...
| `StoragePrecision` | `*string` | No | On-disk rerank-vector format, chosen at create time and immutable. One of `"float32"` (default), `"float16"`, or the TurboQuant tiers `"tq12"` / `"tq8"` / `"tq6"` / `"tq4"` (12/8/6/4 bits per dimension). Prefer the exported constants (`cyborgdb.StoragePrecisionFloat32`, `StoragePrecisionFloat16`, `StoragePrecisionTQ12`, `StoragePrecisionTQ8`, `StoragePrecisionTQ6`, `StoragePrecisionTQ4`). `"float16"` halves storage at a small precision cost; TurboQuant trades storage for a small recall/latency cost, with `"tq4"` the most aggressive (~8× smaller than `float32`, ~94% recall@100). All tiers work with every metric. |
