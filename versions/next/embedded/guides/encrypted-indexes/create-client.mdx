...
For persistence, use a disk-backed or S3 backing store instead of memory. `StorageConfig` exposes three static factories: `memory()`, `disk(path)` (local disk), and `s3(bucket)` (AWS S3 or S3-compatible).
...
// Local persistent storage on disk (pass "" for api_key to run in free-tier mode)
