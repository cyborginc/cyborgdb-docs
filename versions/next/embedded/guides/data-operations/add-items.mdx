...
# Add items to the encrypted index (index_key is required on every data operation)
...
<Note>In Python, every data operation takes `index_key` as a keyword argument — the client does not cache the key across calls. In C++ every key-bearing method takes a trailing `KeyContext`, which constructs implicitly from a bare 32-byte KEK — so `index->Upsert(..., index_key)` is the common path. You can also pass `user_id=` (Python) / a scoped `KeyContext` (C++) for an RBAC user.</Note>
