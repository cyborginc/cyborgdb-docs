...
| `$regex` | `String` | Matches vectors whose metadata string matches the given pattern (see [Regex patterns](#regex-patterns)) |
| `$contains` | `String` | Matches vectors whose metadata string contains the given substring |

### Regex patterns

`$regex` and `$contains` patterns are evaluated by [RE2](https://github.com/google/re2/wiki/Syntax), matched **unanchored** — the same convention as MongoDB's `$regex`. Anchor with `^` and `$` explicitly if you need a full-string match.

RE2 is linear-time and safe against catastrophic-backtracking patterns, but accepts a narrower syntax than PCRE / ECMAScript regex. In particular:

- **Lookahead** and **lookbehind** (`(?=…)`, `(?!…)`, `(?<=…)`, `(?<!…)`) are unsupported.
- **Backreferences** (`\1`, `\2`, …) are unsupported.
- Malformed patterns (e.g. unbalanced groups) raise a validation error.

Any of the above raises `ValueError` at query time — patterns are rejected up front rather than at match time, so a bad pattern fails fast.
