...
CyborgDB supports three backing stores: in-memory (ephemeral), local disk, and S3 (or any S3-compatible store such as MinIO).
...
| `Disk(path, cache_policy)` | Local persistent storage on disk at `path`. Pass a [`CachePolicy`](#cachepolicy) to keep hot data in memory. |
