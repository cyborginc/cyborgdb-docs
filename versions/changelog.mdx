...
- Opt-in per-keystore RAM caches via `CACHE_POLICY_VECTORS`, `CACHE_POLICY_METADATA`, and `CACHE_POLICY_IDS`
- Binary upsert and query endpoints are now documented for the Python client SDK

### Improvements

- `n_lists` is now selected automatically by the core engine
- `train()` defaults to an automatically selected batch size instead of a fixed 2048
- LRU cache for index handles in the service, reducing repeated index-load cost
- Multi-instance deployments can share state through the `s3` backend
- GPU offload for index operations via `GPU_OPERATIONS=upsert,train` (or `all`) on CUDA hosts
- Ingest and API-contract test coverage expanded, including concurrency tests and RBAC/non-RBAC E2E passes across all SDKs
