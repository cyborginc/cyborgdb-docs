...
- Embedded Python `query()` now returns only `id` fields by default when `include` is not specified
  ...
  - Applies to the embedded library only; the REST service retains its `["distance", "metadata"]` default

### Bug Fixes

- Fixed an issue where `query()` always returned distances regardless of the `include` parameter
  - Distances are now correctly treated as optional, matching the existing behavior of metadata
  - Applies to all index types (IVFFlat, IVFPQ, IVFSQ) on both CPU and GPU paths
