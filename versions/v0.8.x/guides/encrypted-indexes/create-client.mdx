...
// Using `redis` storage for the index and config
// Using `postgres` storage for the items
cyborg::LocationConfig index_location(Location::kRedis, "redis://localhost");
cyborg::LocationConfig config_location(Location::kRedis, "redis://localhost");
cyborg::LocationConfig item_location(Location::kPostgres, "host=localhost dbname=postgres");
...
<Warning>`gpu_accelerate` can only be set to `True` if running on a CUDA-enabled system with the CUDA driver installed.</Warning>
