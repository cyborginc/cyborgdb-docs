...
| `const Array2D<ItemID>& ids() const` | `const Array2D<ItemID>&` | Get read-only access to all IDs as a 2D array. |
| `const Array2D<float>& distances() const` | `const Array2D<float>&` | Get read-only access to all distances as a 2D array. |
