...
<Steps>
    ...
    <Step title="Choose Your Database Backend (Optional)">
        For **quick evaluation and testing**, you can skip database setup entirely - the service will automatically use a built-in **redislite** instance.

        For **production deployments** or if you have existing database infrastructure, CyborgDB service supports two external database backends:
        ...
        <Warning>**Redislite is for evaluation and testing only** - For production use, configure an external PostgreSQL or Redis database.</Warning>
    </Step>
    ...
    <Step title="Configure Environment Variables">
        ...
        <Tabs>
            <Tab title="Quick Start (Redislite)">
                <CodeGroup>
                ```bash Linux/macOS
                # Minimal configuration - uses built-in redislite
                export CYBORGDB_API_KEY=cyborg_your_api_key_here
                # No database configuration needed - redislite starts automatically
                ```
                ...
                <Warning>Redislite is for evaluation and testing only. For production, use PostgreSQL or Redis.</Warning>
            </Tab>
    ...
    <Step title="Advanced Configuration">
        ...
        <Tabs>
            <Tab title="Environment Variables">
                ...
                | `CYBORGDB_DB_TYPE` | Database backend type | ⚠️ Optional (defaults to redislite) | `postgres` or `redis` |
                | `CYBORGDB_CONNECTION_STRING` | Database connection details | ⚠️ Optional (defaults to redislite) | See connection formats |
                ...
                <Note>If `CYBORGDB_DB_TYPE` and `CYBORGDB_CONNECTION_STRING` are not set, the service uses a built-in redislite database for evaluation. For production, configure an external database. For more information, refer to [this guide](../advanced/env-vars).</Note>
