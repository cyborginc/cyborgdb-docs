...
<Note>
**Default Behavior:** If you don't set `CYBORGDB_DB_TYPE` or `CYBORGDB_CONNECTION_STRING`, the service automatically uses a built-in **redislite** instance for evaluation and testing. Data is stored in `~/.cyborgdb/redislite.db`.
...
| `CYBORGDB_CONNECTION_STRING` | Database connection details | ⚠️ Optional (defaults to redislite) | See [Connection Formats](#connection-formats) |
...
**Redislite is for EVALUATION ONLY** - For production use, configure an external PostgreSQL or Redis database.
</Warning>
...
<AccordionGroup>
    <Accordion title="Evaluation/Testing Configuration (Redislite)">
        <CodeGroup>
        ...
        <Warning>
        Redislite is for evaluation and testing only. For production, configure an external database.
        </Warning>
    </Accordion>
