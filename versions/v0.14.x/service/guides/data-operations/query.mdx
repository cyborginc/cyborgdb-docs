...
mode: "wide"
canonical: https://docs.cyborg.co/versions/latest/service/guides/data-operations/query
---
...
- `top_k`: the number of results to return. Default: `None` (server typically uses `100`).
...
- `include`: item fields to return (e.g., `["distance", "metadata", "contents", "vector"]`). Default: `None` (includes all available fields). Note: `id` and `distance` are always included.
- `greedy`: whether to perform a greedy search (higher recall but slower). Default: `None` (server determines behavior).
