...
Standalone provides a self-contained database instance (backed by redis-lite) that starts automatically when no external database is configured. Perfect for quick evaluation, development, and testing without setting up external infrastructure.
...
**Redis Compatible**
Uses redis-lite, providing the same Redis protocol and features but running as an embedded instance.
...
# Data stored in ~/.cyborgdb/redislite.db (uses redis-lite backend)
