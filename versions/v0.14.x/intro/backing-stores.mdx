...
**Built-in Redis database for evaluation and testing**

Redislite provides a self-contained Redis instance that starts automatically when no external database is configured. Perfect for quick evaluation, development, and testing without setting up external infrastructure.
...
**Redis Compatible**
Uses the Redis protocol, providing the same API and features as Redis but running as an embedded instance.
...
<Warning>
**Redislite is for EVALUATION PURPOSES ONLY** - It should NOT be used in production environments. For production deployments, use external Redis or PostgreSQL instances with proper persistence, replication, and backup strategies.
</Warning>
...
# Redislite is used automatically when no DB type is set
# No configuration needed - starts automatically in the service
# Data stored in ~/.cyborgdb/redislite.db

# Note: Redislite is only available in CyborgDB Service,
...
| **Redislite** | ~100µs  | Medium     | Local Disk | Eventually Consistent |
