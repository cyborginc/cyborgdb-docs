...
| Parameter | Type | Description |
|-------------------|----------|---------------------------------------------------------------------------------------------|
| `n_lists` | `size_t` | _(Optional)_ Number of inverted lists to create. Defaults to `0`, which auto-determines. |
| `batch_size` | `size_t` | _(Optional)_ Size of each batch for training. Defaults to `2048`. |
| `max_iters` | `size_t` | _(Optional)_ Maximum iterations for training. Defaults to `100`. |
