...
```cpp
IndexIVF(size_t dimension = 0);
```

#### Parameters
| Parameter | Type | Description |
|--------------|-------------------------|---------------------------------------|
| `dimension` | `size_t` | _(Optional)_ Dimensionality of vector embeddings. (will auto-detect if not provided) |
...
```cpp
IndexIVFFlat(size_t dimension = 0);
```
