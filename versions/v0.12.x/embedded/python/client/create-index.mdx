...
| `index_config` | [`IndexConfig`](../types#indexconfig) | _(Optional in overload 2)_ Configuration specifying the index type and parameters. When not provided, uses `IndexIVFFlat()` defaults. |
| `max_cache_size` | `int` | _(Optional)_ Maximum size of local cache to keep for encrypted index. Defaults to `0`. |
| `metric` | [`DistanceMetric`](../types#distancemetric) | _(Optional)_ Distance metric to use for the index. Overrides the metric set in `index_config`. |
