...
async function trainWithMonitoring(
    ...
    try {
        ...
        if (alreadyTrained) {
            console.log('⚠ Index is already trained');
            return {
                success: true,
                duration: 0,
        ...
        if (isNowTrained) {
            console.log(`✅ Training completed successfully in ${duration}ms`);
            console.log('Training result:', result);
        } else {
            console.warn('⚠ Training completed but index may not be fully trained');
        }
        ...
        const duration = Date.now() - startTime;
        console.error(`❌ Training failed after ${duration}ms:`, error.message);
...
async function ensureIndexTrained(
    ...
    try {
        ...
        if (currentlyTrained && !forceRetrain) {
            console.log('✓ Index is already trained and ready for queries');
            return { wasTrained: true, justTrained: false };
        }
        
        if (currentlyTrained && forceRetrain) {
            console.log('🔄 Forcing retraining of already trained index');
        }
        
        if (!currentlyTrained) {
            console.log('⚠ Index requires training before optimal querying');
        }
