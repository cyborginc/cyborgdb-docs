...
The `CreateIndexRequest` class defines the parameters for creating a new encrypted index.
...
Base class for operations that require index identification and authentication.
...
Class for adding or updating vectors in an encrypted index.
...
Class for performing similarity search in the encrypted index.
...
Class for performing batch similarity searches with multiple vectors.
...
Class for training an index to optimize query performance.
...
Class for deleting vectors from the encrypted index.
...
Class for retrieving specific vectors from the index.
...
All types are fully typed for TypeScript environments with:

- **Strict typing**: All properties have specific types with null safety
- **IntelliSense support**: Auto-completion and type checking in compatible IDEs
- **Compile-time validation**: Catch type errors before runtime
- **Optional/Required distinction**: Clear indication of which fields are required

### Example with Type Safety

```typescript
import { CreateIndexRequest, IndexIVFPQ, VectorItem } from 'cyborgdb';
...
const request: CreateIndexRequest = {
    ...
    indexKey: 'valid-hex-string',
    indexName: 'my-index',
    embeddingModel: null  // Explicitly set to null if not using
};

// TypeScript will catch errors like missing required fields
const vectorItem: VectorItem = {
    id: 'doc1',
    vector: [0.1, 0.2, 0.3],
    // contents and metadata are optional
};
```
