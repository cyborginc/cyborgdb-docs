...
|---------------|--------|---------|-------------------------------------------------------|
| `n_lists` | `int` | `null` (auto) | _(Optional)_ Number of inverted index lists to create in the index. When `null` or omitted, auto-determines based on the number of vectors in the index. |
...
| `max_memory` | `int` | `0` | _(Optional)_ Maximum memory usage in MB. `0` sets no limit. |
