...
// Create a client
client := cyborgdb.NewClient("http://localhost:8000", "your-api-key")

// Create the IVFFlat index config
indexConfig := cyborgdb.IndexIVFFlat{}

// Generate encryption key and create the index
indexName := "test_index"
indexKey := client.GenerateKey() // Generate secure 32-byte key

index, err := client.CreateIndex(context.Background(), cyborgdb.CreateIndexRequest{
    IndexName:   indexName,
    IndexKey:    indexKey,
...
// Set index parameters
pqDim := 32  // Dimension must be divisible by pqDim
pqBits := 8  // Number of bits for each pq dimension

// Create the IVFPQ index config
indexConfig := cyborgdb.IndexIVFPQ{
    PqDim:  pqDim,
    PqBits: pqBits,
}
