...
- `n_probes`: the number of clusters to search for each query vector (default: auto-determined).
- `filters`: metadata filters to apply to the query (dictionary format).
- `include`: item fields to return (e.g., `["distance", "metadata"]`). Default: `["distance", "metadata"]`.
- `greedy`: whether to perform a greedy search (higher recall but slower). Default: `false`.
