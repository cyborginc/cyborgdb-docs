...
<Update label="v0.17.0" description="2026-06-25">
## CyborgDB v0.17.0
...
- **Storage refactor.** The backing store surface is now just `memory`, `disk` (embedded RocksDB), or `s3`
  ...
  - `CYBORGDB_CONNECTION_STRING` is replaced by `CYBORGDB_DISK_PATH` and the `CYBORGDB_S3_*` variables — switch `standalone` → `disk` when upgrading
...
- CyborgDB Client SDKs in **early access**
    ...
    - JavaScript
    - TypeScript
- [LangChain integration for CyborgDB](/versions/latest/integrations/langchain/introduction) now available
...
- Fixed a bug where [automatic embedding generation](/versions/latest/embedded/guides/encrypted-indexes/create-index#automatic-embedding-generation) would not load the model correctly when using [`load_index()`](/versions/latest/embedded/guides/encrypted-indexes/load-index)
- Fixed a bug which caused intermittent [`query()`](/versions/latest/embedded/guides/data-operations/query) recall issues on Linux
- Fixed incorrect type returns for [`get()`](/versions/latest/embedded/guides/data-operations/get-items) in C++ and Python APIs

### Improvements

- Enabled pre-filtering on [metadata queries](/versions/latest/embedded/guides/data-operations/metadata-filtering) to improve performance
...
- Added support for **metadata fields & query filtering**
    - Metadata fields can be added to items during [`upsert()`](/versions/latest/embedded/guides/data-operations/add-items#adding-items-with-metadata) calls
    - [`query()`](/versions/latest/embedded/guides/data-operations/metadata-filtering) calls can filter results based on metadata fields
- Added support for **item deletion**
    - [`delete()`](/versions/latest/embedded/guides/data-operations/delete-items) calls can remove items from the encrypted index
    - [`upsert()`](/versions/latest/embedded/guides/data-operations/add-items) conflicts are now resolved by overwriting existing items
- Added support for **automatic embedding generation**
    - An embedding model can be specified during [index creation](/versions/latest/embedded/guides/encrypted-indexes/create-index#automatic-embedding-generation)
    - [`upsert()`](/versions/latest/embedded/guides/data-operations/add-items#automatic-embedding-generation) calls can generate embeddings directly from index contents
    - [`query()`](/versions/latest/embedded/guides/data-operations/query#automatic-embedding-generation) calls can use the model to generate embeddings for query vectors

### Breaking Changes

- IDs are now `string`-type (instead of `int`)
    - This impacts `upsert()`, `get()` and `query()` function signatures
- `get_item()` and `get_items()` have been replaced by [`get()`](/versions/latest/embedded/guides/data-operations/get-items)
...
- Added support for **encrypted item storage & retrieval**
    ...
    - [`upsert()`](/versions/latest/embedded/guides/data-operations/add-items) calls can take item content as bytes, encrypt and store them in the index
    - [`get_item()`](/versions/latest/embedded/guides/data-operations/get-items) calls can retrieve encrypted item content and decrypt them
- Client-side caching now available for encrypted indexes (via [`max_cache_size`](/versions/latest/embedded/guides/encrypted-indexes/create-index) parameter)
...
- Python & C++ APIs now split into two classes:
    - [`Client`](/versions/latest/embedded/guides/encrypted-indexes/create-client) class now handles DB backend connection, index creation and loading
    - [`EncryptedIndex`](/versions/latest/embedded/guides/encrypted-indexes/create-index) class now handles data operations (e.g., `upsert()`, `query()`, etc.)
